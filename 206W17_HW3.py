...
## SI 206 - W17 - HW3
## COMMENT WITH:
## Your section day/time: Emma Aboukasm, Friday 9am
...
## Use regular expressions to define a function called parse_counted_words which should accept a string, find strings of the form: 
## <count> <word>  e.g. 101 Dalmations, inside it, and should return either the LAST correctly matching number-word combo 
## in the string as a tuple, e.g. ('13', "pineapples"), if there are any such sub-strings, or None, if there are not.
...
def parse_counted_words(string):
    list_of_tups = []
    x = re.findall(r'\b([0-9]+)\s(.[\A-Za-z]+[^.\s\!])', string)
